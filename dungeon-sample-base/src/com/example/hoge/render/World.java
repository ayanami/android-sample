package com.example.hoge.render;
...
import com.example.hoge.objects.AbstractDynamicObject;
import com.example.hoge.objects.Cube;
import com.example.hoge.objects.DominoPlate;
import com.example.hoge.objects.Ground;
import com.example.hoge.objects.Sky;
import com.example.hoge.tool.PhysicsUtil;
import com.example.hoge.tool.RenderUtil;
...
//            ;
    ...
    public void draw(GL10 gl) {
        this.drawObjcets(gl);
        synchronized (this.dynamicsWorld) {
            this.dynamicsWorld.stepSimulation(0.33f);
        }
    }

    private void drawObjcets(GL10 gl) {
        Cube.bind(gl);
        for (int i = 0; i < MAX_CUBE_BULLET && this.isShoot && this.hasCBullet(i); i++)
            this.cubeBullets[i].draw(gl);
        for (int i = 0; i < CUBE_LENGTH && this.hasDefCube(i); i++)
            this.cubes[i].draw(gl);
        Cube.unBind(gl);

        DominoPlate.bind(gl);
        for (int i = 0; i < DOMINO_PLATE_LENGTH && this.hasDomino(i); i++)
            this.dominoPlates[i].draw(gl);
    ...
    public void shootCube(Vector3f linVel, Vector3f genPos) {
        ...
        if (cShootBullet >= MAX_CUBE_BULLET)
            cShootBullet = 0;
    ...
    public AbstractDynamicObject[] getAllObjects() {
        ...
        int cnt = 0;
        for (int i = 0; i < CUBE_LENGTH; i++)
            allObjs[cnt++] = (AbstractDynamicObject)this.cubes[i];
        for (int i = 0; i < MAX_CUBE_BULLET; i++)
            allObjs[cnt++] = (AbstractDynamicObject)this.cubeBullets[i];
        for (int i = 0; i < DOMINO_PLATE_LENGTH; i++)
            allObjs[cnt++] = (AbstractDynamicObject)this.dominoPlates[i];
    ...
    private class GenObjectHandler extends Handler implements Runnable {
        ...
        public void run() {
            ...
            if (isDefObjNull)
                this.postDelayed(this, this.period);
        }
    }
